...
class TableRepositories:
    def __init__(self, open_cv_image, rect, SORT_METHOD='top-to-bottom', MAX_THRESHOLD_VALUE=255, BLOCK_SIZE=15,
                 THRESHOLD_CONSTANT=0, SCALE=15):
        '''
        :param open_cv_image: absolute path of input image file
    ...
    def load_image(self):
        ...
        IMAGE_BUFFER = 10
        image = self.open_cv_image
        self.open_cv_image = image  # [self.rect['y']-IMAGE_BUFFER:self.rect['y']+self.rect['h']+IMAGE_BUFFER,self.rect['x']-IMAGE_BUFFER:self.rect['x']+self.rect['w']+IMAGE_BUFFER]
        self.slate = np.zeros (self.open_cv_image.shape)

    def get_table_mask(self):
        #binarization of image
        gray                 = cv2.cvtColor(~self.open_cv_image, cv2.COLOR_BGR2GRAY)
        filtered = cv2.adaptiveThreshold (gray, self.MAX_THRESHOLD_VALUE, cv2.ADAPTIVE_THRESH_MEAN_C,
        ...
        #height_to_width_ratio = self.open_cv_image.shape[0] / float(self.open_cv_image.shape[1])
            ...
            if (area_ratio < 0.8) & (area_ratio > 0.005):
                ...
                if len (midpoints) > 1:
                    ...
                    if shift < 10:
                        ...
                        yi = 0
                        xi = xi + 1
                rects.append ({"x": x1, "y": y1, "w": w1, "h": h1, "row": xi, "col": yi})
                ...
                #cv2.imwrite('/tmp/slate' + str(i) + '.png' , draw_conts)
    ...
    def end_point_correction(self,x,y,w,h,margin):
        ...
        ymax = self.open_cv_image.shape [0]
        xmax = self.open_cv_image.shape [1]
    ...
    def table_indexing(self):
        ...
        # list_of_tables = []
        image_area = float (self.open_cv_image.shape [0] * self.open_cv_image.shape [1])
        ...
        #cv2.imwrite ('/tmp/slate.png', self.slate)
        #cv2.imwrite ('/tmp/mask.png', self.mask)
        #cv2.imwrite ('/tmp/filtered.png', self.filtered)
